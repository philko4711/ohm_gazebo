...
// Verändert husky_description in ..._new
	int argc = 0;
	char** argv = NULL;
	ros::init(argc, argv, "husky_description_new", ros::init_options::NoSigintHandler | ros::init_options::AnonymousName);
...
void DiffDrivePlugin::UpdateChild()
{
	...
	double vl, vr; // Definition Geschwindigkeit
	double d1, d2, d3, d4;
	double d_avg_left, d_avg_right;
	double dr, da;
	double _slip_correction;   // durch Test ermittelt 0.678 
   ...
   _slip_correction     = _slip_correction * _slip_correction;
	...
	da = (d_avg_right - d_avg_left) / ws * _slip_correction;


// Odom ueber Vel analog RT4
//	vr = wd / 2 * joints[FRONTLEFT]->GetVelocity(0);
//	vl = wd / 2 * joints [FRONTRIGHT]->GetVelocity(0);

//  dr     = (vl + vr) / 2.0;
//  da     = (vr - vl) / ws * _slip_correction;
...
//	Edit (Odom ueber Vel analog RT4)
//	wheelOdomPose[0] += dr * cos(wheelOdomPose[2]) * stepTime.Double();
//	wheelOdomPose[1] += dr * sin(wheelOdomPose[2]) * stepTime.Double();
//	wheelOdomPose[2] += da * stepTime.Double();
